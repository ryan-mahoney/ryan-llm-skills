---
name: spec-run-glm
description: Use this OpenCode-specific variant when the user asks to run a spec with GLM 5.2 planning/orchestration and GLM 5.2 Fast (firepass) implementation. It follows spec-run semantics, but routes planning and implementation through specialized OpenCode agents.
mode: coding
scope: document
...
## GLM OpenCode Routing

Use the global OpenCode agents configured in `~/.config/opencode/opencode.jsonc`:

- `spec-run-glm-orchestrator`: primary orchestration agent using
  `z-ai/glm-5.2`.
- `spec-run-glm-planner`: subagent for per-step subspec planning using
  `z-ai/glm-5.2`.
- `spec-run-glm-implementer`: subagent for implementation and fix-ups using
...
subagent behavior and report that the GLM 5.2 / GLM 5.2 Fast split was unavailable.

## Per-Step Flow

For each implementation step:

1. Run `spec-run-glm-planner` with the Planning Prompt below.
2. If the planner reports a hard blocker or spec/code mismatch, treat it as a
   spec defect and stop before implementation.
3. Run `spec-run-glm-implementer` with the Implementation Prompt below.
...
If verification fails, use `spec-run-glm-implementer` for fix-ups. The
