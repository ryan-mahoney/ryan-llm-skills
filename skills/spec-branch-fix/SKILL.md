...
Apply one iteration of the branch correctness review. `spec-branch-review` wrote
...
- **Spec.** As `spec-branch-review` resolves it: `spec=<path>` / `.specs/<slug>/`
  folder, else the conversation, else the most recently modified `.specs/*/spec.md`.
  `<spec-dir>` is the folder containing it.
...
- A branch fix may legitimately span files from several steps — that is expected,
  since the branch review catches cross-step integration bugs the per-step reviews
...
The file leads with a fenced `fix:` YAML block — the machine-readable record the
loop driver parses — followed by prose. Every dismissed finding must carry its
`signature` and `dismissal` class.
