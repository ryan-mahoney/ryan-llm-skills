...
const displayLabel = (value = "") => ({
  proven: "verified", unproven: "not verified", partial: "partly verified",
  merge: "before merge", deploy: "at deployment", "post-deploy": "after deployment",
  pass: "passed", granted: "approved", "not-requested": "not requested",
  "not-assessed": "not assessed", "not-applicable": "not applicable", "not-run": "not run",
...
const resolveArtifact = (rawValue) => {
  ...
  // Internal provenance stays in the manifest, not in reader-facing links.
  if (path.isAbsolute(filePart) || filePart.split(/[\\/]/).includes(".specs")) {
    return { href: null, exists: false, image: false };
  }
  const candidates = [
  ...
  // A tour can carry supporting files beside it. Other local references must
  // exist in the reviewed commit, rather than only in the author's checkout.
  const included = relativeTarget !== ".." && !relativeTarget.startsWith(`..${path.sep}`) && !path.isAbsolute(relativeTarget);
  if (!included) {
    const root = spawnSync("git", ["-C", path.dirname(target), "rev-parse", "--show-toplevel"], { encoding: "utf8" });
    if (root.status !== 0) return { href: null, exists: false, image: false };
    const repoPath = path.relative(root.stdout.trim(), realpathSync(target)).split(path.sep).join("/");
    const committed = spawnSync("git", ["-C", root.stdout.trim(), "cat-file", "-e", `${manifest.commit}:${repoPath}`], { encoding: "utf8" });
    if (committed.status !== 0) return { href: null, exists: false, image: false };
  }
  const href = `${relativeTarget.split(path.sep).join("/") || path.basename(target)}${fragment}`;
  return { label: path.basename(filePart), href, exists: true, image: /\.(png|jpe?g|gif|webp)$/i.test(filePart) };
};

const artifactLink = (value, label) => {
  const artifact = resolveArtifact(value);
  if (!artifact.href) return '<span class="muted">Supporting file not shared.</span>';
...
const renderArtifactSet = (artifacts, scenarioTitle) => {
  ...
  return `<div class="artifact-set">${artifacts.map((value) => {
    const artifact = resolveArtifact(value);
    if (artifact.image && artifact.href) return `<figure class="qa-image"><a href="${attr(artifact.href)}" target="_blank" rel="noreferrer"><img src="${attr(artifact.href)}" alt="${attr(scenarioTitle)}" loading="lazy"></a><figcaption>${escapeHtml(artifact.label)}</figcaption></figure>`;
    return `<span>${artifactLink(value)}</span>`;
  }).join("")}</div>`;
};

const attentionItems = [
  ...gaps.map((gap) => ({ level: "blocking", kind: "Missing verification", title: gap })),
  ...nonPassingRequired.map((gate) => ({ level: "blocking", kind: `${displayLabel(gate.phase)} · required check`, title: `${displayLabel(gate.status)}: ${gate.boundary}`, claim: gate.claims[0] })),
  ...nonProvenClaims.map((claim) => ({ level: claim.phase === "merge" ? "blocking" : "follow-up", kind: `${displayLabel(claim.phase)}`, title: `${displayLabel(claim.status)}: ${claim.statement}`, claim: claim.id })),
  ...(audit.verdict === "pass" && audit.commit === manifest.commit ? [] : [{ level: "blocking", kind: "Independent review", title: "The independent review has not passed for this commit." }]),
  ...nonPassingOptional.filter((gate) => !nonProvenClaims.some((claim) => claim.gates.includes(gate.id))).map((gate) => ({ level: "follow-up", kind: `${displayLabel(gate.phase)} · optional check`, title: `${displayLabel(gate.status)}: ${gate.boundary}`, claim: gate.claims[0] })),
  ...laterGates.filter((gate) => gate.status !== "passed" && !nonProvenClaims.some((claim) => claim.gates.includes(gate.id))).map((gate) => ({ level: "follow-up", kind: `${displayLabel(gate.phase)} · required check`, title: `${displayLabel(gate.status)}: ${gate.boundary}`, claim: gate.claims[0] })),
  ...deploymentGaps.map((gap) => ({ level: "follow-up", kind: "Deployment issue", title: gap })),
  ...(deployment.authorization === "required" ? [{ level: "follow-up", kind: "Authorization required", title: "Deployment approval is still required." }] : []),
  ...residualRisks.map((risk) => ({ level: "follow-up", kind: "Known risk", title: risk })),
];

const attentionHtml = attentionItems.length
  ? `<div class="attention-list">${attentionItems.map((item) => `<article class="attention-item attention-item--${attr(item.level)}"><span class="attention-kind">${escapeHtml(item.kind)}</span><p>${escapeHtml(item.title)}</p>${item.claim ? `<button type="button" class="text-action" data-claim-select="${attr(item.claim)}">View results</button>` : ""}</article>`).join("")}</div>`
  : '<p class="clear-state"><strong>No outstanding issues recorded.</strong></p>';
...
<title>${escapeHtml(manifest.title)} — Change overview</title><style>${styles}</style></head><body onbeforeprint="document.querySelectorAll('details').forEach(function(d){d.dataset.printWasOpen=d.open?'1':'0';d.open=true})" onafterprint="document.querySelectorAll('details').forEach(function(d){d.open=d.dataset.printWasOpen==='1';delete d.dataset.printWasOpen})"><a class="skip" href="#attention">Skip to outstanding issues</a>
<header class="tour-header"><div class="shell"><p class="section-kicker">Change overview</p><h1>${escapeHtml(manifest.title)}</h1><div class="verdict-row"><span class="verdict verdict--${attr(manifest.verdict)}">${legacy ? "Earlier report" : "Pre-merge verification"} · ${escapeHtml(manifest.verdict)}</span><span class="commit-line">Commit <code>${escapeHtml(manifest.commit)}</code></span></div><p class="commit-line">Deployment approval: <strong>${escapeHtml(displayLabel(deployment.authorization))}</strong> · Checks after deployment: <strong>${escapeHtml(displayLabel(deployment.postDeploy))}</strong></p>${legacy ? '<p role="note"><strong>Older report format.</strong> Project context and deployment permission were not recorded. These statuses have not been verified under the current checks.</p>' : ""}<div class="rationale"><p>${escapeHtml(manifest.summary)}</p></div><div class="meta-line"><span>${escapeHtml(manifest.branch)} → ${escapeHtml(manifest.base)}</span><span>Generated ${escapeHtml(manifest.generatedAt)}</span></div><dl class="status-ledger"><div><dt>Required checks</dt><dd class="${passedRequiredCount === requiredGates.length ? "status--passed" : "status--failed"}">${passedRequiredCount}/${requiredGates.length} passed</dd></div><div><dt>Verified behavior</dt><dd class="${provenCount === mergeClaims.length ? "status--proven" : "status--partial"}">${provenCount}/${mergeClaims.length} verified</dd></div><div><dt>Independent review</dt><dd class="${audit.verdict === "pass" ? "status--passed" : "status--failed"}">${escapeHtml(displayLabel(audit.verdict || "missing"))}</dd></div><div><dt>Deployment</dt><dd class="${deploymentStatusClass}">${escapeHtml(displayLabel(deployment.readiness))}</dd></div><div><dt>Outstanding issues</dt><dd class="${attentionItems.length ? "status--stale" : "status--passed"}">${attentionItems.length} item${attentionItems.length === 1 ? "" : "s"}</dd></div></dl></div></header>
<nav class="tour-nav" aria-label="Report sections"><div class="shell"><a href="#attention">Issues</a><a href="#context">Background</a><a href="#architecture">What changed</a><a href="#proof">Verification</a><a href="#qa">Test scenarios</a><a href="#deployment">Deployment</a><a href="#implementation">Commits</a></div></nav>
<main><section id="attention" class="tour-section"><div class="shell"><div class="section-heading"><h2>Outstanding issues</h2></div>${attentionHtml}</div></section>
<section id="context" class="tour-section"><div class="shell"><div class="section-heading"><h2>Background</h2></div><p>${escapeHtml(context.summary)}</p><div class="operations-grid"><section><h3>Decisions</h3>${renderList(context.decisions)}</section><section><h3>Scope limits</h3>${renderList(context.omissions)}</section></div><div class="secondary-details"><h3>Maintenance</h3>${renderList(context.burden)}</div></div></section>
<section id="architecture" class="tour-section"><div class="shell"><div class="section-heading"><h2>What changed</h2></div><div class="before-after"><article class="architecture-cell"><h3>Before</h3><p>${escapeHtml(architecture.before || "Not recorded.")}</p></article><article class="architecture-cell"><h3>After</h3><p>${escapeHtml(architecture.after || "Not recorded.")}</p></article></div>${(architecture.boundaries || []).map((boundary) => `<div class="boundary-line"><strong>Data flow</strong><code>${escapeHtml(boundary)}</code></div>`).join("")}${(architecture.decisions || []).length ? `<details class="secondary-details"><summary>${architecture.decisions.length} architecture decision${architecture.decisions.length === 1 ? "" : "s"}</summary>${architecture.decisions.map((decision) => `<article class="decision-row"><strong>${escapeHtml(decision.decision)}</strong><p>${escapeHtml(decision.reason)}</p><span>${artifactLink(decision.source, "Decision reference")}</span></article>`).join("")}</details>` : ""}</div></section>
<section id="proof" class="tour-section"><div class="shell"><div class="section-heading"><h2>Verification</h2><p>Select a behavior to see its checks, results, and limits.</p></div><div class="proof-layout"><div class="claim-list" aria-label="Verified behaviors">${claimButtons}</div><div class="claim-inspector" aria-live="polite">${claimPanels}</div></div></div></section>
<section id="qa" class="tour-section"><div class="shell"><div class="section-heading"><h2>Test scenarios</h2></div><div class="entrypoints">${(qa.entrypoints || []).map((entry) => `<article class="entrypoint"><strong>${escapeHtml(entry.label)}</strong><p>${escapeHtml(entry.location)}</p><p class="muted">${escapeHtml(entry.setup)}</p></article>`).join("") || '<p class="muted">No starting points recorded.</p>'}</div>${(qa.scenarios || []).length ? `<div class="qa-layout"><div class="scenario-list" aria-label="QA scenarios">${qaButtons}</div><div class="scenario-inspector" aria-live="polite">${qaPanels}</div></div>` : '<p class="muted">No test scenarios recorded.</p>'}</div></section>
<section id="deployment" class="tour-section"><div class="shell"><div class="section-heading"><h2>Deployment</h2><p><span class="${deploymentStatusClass}">${escapeHtml(displayLabel(deployment.readiness))}</span> at <code>${escapeHtml(shortCommit(manifest.commit))}</code>.</p></div><p>Approval: <strong>${escapeHtml(displayLabel(deployment.authorization))}</strong> · Source: ${escapeHtml(deployment.authorizationSource)}. Checks after deployment: <strong>${escapeHtml(displayLabel(deployment.postDeploy))}</strong>.</p><div class="deployment-grid"><article class="deployment-fact"><h3>Migrations</h3><p>${escapeHtml(deployment.migrations)}</p></article><article class="deployment-fact"><h3>Configuration</h3><p>${escapeHtml(deployment.configuration)}</p></article><article class="deployment-fact"><h3>Rollback or forward fix</h3><p>${escapeHtml(deployment.rollback)}</p></article></div><div class="operations-grid"><section><h3>Checks after deployment</h3>${renderList(deployment.observability || [], "No checks recorded.")}</section><section><h3>Known risks</h3><p>${residualRisks.length ? `${residualRisks.length} item${residualRisks.length === 1 ? "" : "s"} listed under Outstanding issues.` : "None recorded."}</p></section></div><div class="audit-line"><strong>Independent review · ${escapeHtml(displayLabel(audit.verdict || "missing"))}</strong><span>Commit <code>${escapeHtml(shortCommit(audit.commit || "unbound"))}</code></span>${artifactLink(audit.artifact || "", "Review details")}</div></div></section>
<section id="implementation" class="tour-section"><div class="shell"><details class="secondary-details"><summary>Commits</summary>${implementationRows || '<p class="muted">No commits recorded.</p>'}</details></div></section></main>
