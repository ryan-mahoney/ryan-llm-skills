...
description: Use this OpenCode-specific variant when the user asks to run a spec with GLM 5.2 planning/orchestration and DeepSeek V4 Flash implementation. It follows spec-run semantics, but routes planning through GLM 5.2 and implementation through DeepSeek V4 Flash.
mode: coding
scope: document
...
## GLM DeepSeek OpenCode Routing

Use the global OpenCode agents configured in `~/.config/opencode/opencode.jsonc`:

- `spec-run-glm-deepseek-orchestrator`: primary orchestration agent using
  `zai-coding-plan/glm-5.2`.
- `spec-run-glm-deepseek-planner`: subagent for per-step subspec planning using
  `zai-coding-plan/glm-5.2`.
- `spec-run-glm-deepseek-implementer`: subagent for implementation and fix-ups using
  `deepseek/deepseek-v4-flash`.

If those specialized agents are unavailable, fall back to canonical `spec-run`
subagent behavior and report that the GLM 5.2 / DeepSeek V4 Flash split was unavailable.

## Per-Step Flow

For each implementation step:

1. Run `spec-run-glm-deepseek-planner` with the Planning Prompt below.
2. If the planner reports a hard blocker or spec/code mismatch, treat it as a
   spec defect and stop before implementation.
3. Run `spec-run-glm-deepseek-implementer` with the Implementation Prompt below.
...
If verification fails, use `spec-run-glm-deepseek-implementer` for fix-ups. The
