...
The spec-driven workflow solves a different problem: produce correct code from a reviewed specification. Human artifacts live in an external feature document folder and machine state under its `.restory/spec/` directory. An architect proposes; critics stress-test; a writer commits to numbered steps; preparation code-grounds the spec, derives prose guardrails, and plans each step; a runner consumes those immutable plans one commit at a time; one final branch loop reviews and fixes the integrated result. Every stage exchanges explicit files. Nothing depends on hidden in-memory state between stages.
